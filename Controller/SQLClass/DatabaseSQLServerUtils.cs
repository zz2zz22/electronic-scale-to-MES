...
namespace ElectronicScale2MES
{
    class DatabaseSQLServerUtils
    {
        ...
        public static MySqlConnection TestMySQL(string host, string user, string password)
        {
        ...
        public static SqlConnection TestMSSQL(string datasource, string username, string password)
        {
            string connectionString = "";
            if (Properties.Settings.Default.conHostType == 0)
            {
                connectionString = @"Data Source=" + datasource + ";Persist Security Info=True;User ID=" + username + ";Password=" + password;
            }
            else
            {
